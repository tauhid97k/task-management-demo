...
const generateTasksForAllTemplates = () => {
  ...
  ["DFP120", "DFP240", "DFP270", "DFP360"].forEach((packageId) => {
    Object.keys(mockPendingTasks).forEach((templateId) => {
      ...
      allTasks[newTemplateId] = mockPendingTasks[
        templateId as keyof typeof mockPendingTasks
      ].map((task) => ({
        ...task,
  ...
  const pendingTasks =
    templateId && allPendingTasks[templateId]
      ? allPendingTasks[templateId].filter(
          (task: Task) =>
            task.category === activeTab.toLowerCase() || activeTab === "all"
        )
      : [];
            ...
            <div className="max-h-[300px] overflow-y-auto border rounded-md p-2">
              {pendingTasks.length > 0 ? (
                ...
                <Alert className="bg-amber-50 text-amber-800 border-amber-200">
                  ...
                  <AlertDescription>
                    No pending tasks available for this category. Try selecting
