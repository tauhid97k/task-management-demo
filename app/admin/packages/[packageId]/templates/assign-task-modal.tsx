...
const generateTasksForAllTemplates = () => {
  ...
  Object.keys(mockPendingTasks).forEach((templateId) => {
    allTasks[templateId] =
      mockPendingTasks[templateId as keyof typeof mockPendingTasks];
  });

  // Generate for other package templates
  ["DFP120", "DFP240", "DFP270", "DFP360"].forEach((packageId) => {
    [1, 2, 3].forEach((templateNum) => {
      const templateId = `${packageId}-template-${templateNum}`;
      // Copy tasks from corresponding DFP90 template with modified IDs
      const baseTemplateId = `DFP90-template-${templateNum}`;
      if (mockPendingTasks[baseTemplateId as keyof typeof mockPendingTasks]) {
        allTasks[templateId] = mockPendingTasks[
          baseTemplateId as keyof typeof mockPendingTasks
        ].map((task) => ({
          ...task,
...
export function AssignTaskModal({
  ...
  const pendingTasks = templateId ? allPendingTasks[templateId] || [] : [];
  ...
  const handleAssignTasks = () => {
    ...
    console.log(
      `Assigned tasks ${selectedTasks.join(
        ", "
      )} from template ${templateId} to team member ${selectedMember?.id}`
    );
    onClose();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>
            {step === 1 ? "Select Team Member" : "Assign Tasks"}
          </DialogTitle>
        </DialogHeader>
        ...
        {step === 1 ? (
          ...
          <>
            ...
            <div className="space-y-1 mb-4">
              ...
              <p className="text-sm text-muted-foreground">
                Select tasks to assign to this team member
              </p>
            </div>
